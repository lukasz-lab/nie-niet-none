...
class Operation extends operation_trait_1.OperationTrait {
    ...
    channels() {
        ...
        if (this._json.channel) {
            for (const [channelName, channel] of Object.entries(((_a = this._meta.asyncapi) === null || _a === void 0 ? void 0 : _a.parsed.channels) || {})) {
                if (channel === this._json.channel) {
                    return new channels_1.Channels([
                        this.createModel(channel_1.Channel, this._json.channel, { id: channelName, pointer: `/channels/${channelName}` })
                    ]);
                }
            }
        }
    ...
    messages() {
        ...
        if (Array.isArray(this._json.messages)) {
            this._json.messages.forEach((message, index) => {
                messages.push(this.createModel(message_1.Message, message, { id: '', pointer: this.jsonPath(`messages/${index}`) }));
            });
